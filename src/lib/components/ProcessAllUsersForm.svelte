<script lang="ts">
	...
	async function getUsersToProcess(event: SubmitEvent) {
		...
		try {
			const url = new URL('/api/get_new_users', window.location.origin);
			if (reprocessDate) {
				url.searchParams.set('reprocessDate', reprocessDate);
			}
...
<div class="mx-auto w-full max-w-2xl p-4">
	...
	{#if step === 1}
		...
		<form onsubmit={getUsersToProcess} class="flex flex-col gap-4">
			...
			<div class="flex flex-col gap-2">
				<label for="reprocessDate"> Reprocess users created before (optional): </label>
				<input id="reprocessDate" type="date" bind:value={reprocessDate} />
				<p class="text-sm text-stone-600">
					Leave empty to only process new users not in the database.
				</p>
			</div>
		...
		<form onsubmit={startProcessing} class="flex flex-col gap-4">
			...
			<div class="max-h-60 overflow-y-auto rounded-md border border-stone-200 p-4">
				{#each userList as user}
					<div class="py-1">@{user.username} ({user.displayName})</div>
				{/each}
			</div>
